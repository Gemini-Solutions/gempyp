...
def checkCmdLineParams(args): 
    try:
        ...
        if args.config:
            ...
            print("Config file not passed checking commandline variable")
            paramMissing =False
            if not args.srcDB:
...
def main():
    try:
        ...
        if r_val == 0:
            ...
            if r_val == 0:
                config_data = configuratorObj.get_config()
                ...
                if config_list:
                    process_no = max(len(config_list),2) #taking it 2 for locla machine will increase later
                    pool = Pool(processes = process_no)
                    if process_no>1:
                        for res in pool.imap_unordered(worker_process,config_list):
                            res = list(res)
