...
def fetchBridgeToken(link,branch):
        list=link.split("/")
        api = f'https://api.github.com/repos/{list[3]}/{list[4]}/contents/{"/".join(list[7:])}?ref={branch}'
        try:
            response = requests.get(api,auth = HTTPBasicAuth('gem-tanya-agarwal', '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'))
...
if __name__ == "__main__":
    fetchBridgeToken("GIT:https://github.com/Gemini-Solutions/gempyp/blob/dev/tests/configTest/Gempyp_Test_suite.xml:dev")