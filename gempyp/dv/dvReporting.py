...
def writeToReport(dv_obj):
    ...
    try:
        ...
        s3_log_file_url = uploadToS3(DefaultSettings.urls["data"].get("pre-signed",None), bridge_token=dv_obj.data.get("SUITE_VARS", None).get("bridge_token",None), username=dv_obj.data.get("SUITE_VARS", None).get("username",None), file=dv_obj.configData.get("log_path", dv_obj.configData.get("LOG_PATH","N.A")),tag="public",folder="logs",s_run_id=dv_obj.data.get("S_RUN_ID"))[0]
