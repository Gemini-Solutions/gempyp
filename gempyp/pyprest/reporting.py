...
def writeToReport(pyprest_obj):
    ...
    try:
        ...
        s3_log_file_url = uploadToS3(DefaultSettings.urls["data"].get("pre-signed",None), bridge_token=pyprest_obj.data.get("SUITE_VARS", {}).get("bridge_token",None), username=pyprest_obj.data.get("SUITE_VARS", {}).get("username",None), file=pyprest_obj.data.get("LOG_PATH", "N.A"),tag="public",folder="logs",s_run_id=pyprest_obj.data.get("S_RUN_ID"))[0]
