...
class Common:
    ...
    def makeSuiteDetails(self, plugin_data:dict={})->None:
            ...
            run_mode = "LINUX_CLI"
            # self.reporter = TestcaseReporter(self.dataFromIni["PROJECT_NAME"], rep.nodeid)
            if os.name == 'nt':
                run_mode = "WINDOWS"
            Suite_details = {
    ...
    def getTestcaseData(self, testcaseData:dict)->None:
            ...
            self.status=self.reporter.Status = status
            projectName = self.data["PROJECT_NAME"]=self.dataFromIni.get("project-name","")
            env = self.data["ENVIRONMENT"]=self.dataFromIni.get("environment","beta")
